   ...
   return (
    <div className="w-full h-12 flex items-center px-4 ml-60 z-10 fixed top-0 border-b bg-[#161616] border-b-[#303030]">
     {editing ? <div className="flex gap-2 items-center">
        <Input onKeyUp={async(evt) => {
          if(evt.code === "Enter") await handleEdit();
        }} value={noteTitle} onChange={(evt) => setNoteTitle(evt.target.value)} placeholder="Note title" aria-label="Note title"  />
        <Button onClick={async() => await handleEdit()} color="primary">Save title</Button>
